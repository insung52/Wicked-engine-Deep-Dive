...
매 프레임마다 probe 에서 ray 를 발사 -> 주변 radiance 를 캡처
캡처된 정보를 probe 마다 저장 -> 이후 셰이딩 시 또는 radiance 캡처 시 재귀적으로 사용

매 프레임마다 probe 가 주변으로 ray tracing -> 주변 radiance 를 캡처
캡처된 정보를 probe 에 SH 로 저장 -> 이후 셰이딩 또는 probe 의 ray tracing 에 재귀적으로 사용 -> 무한 반사를 근사
...
첫 프레임 ddgi cs : probe 가 ray 를 균일하게 주변으로 발사해, 각 ray의 충돌지점 표면의 직접광을 shadow ray 를 이용해 계산한다. (랜덤하게 하나의 조명만 선택)
종합된 결과를 sh 형태로 기록한다.

첫 프레임 ps : 현재 픽셀의 월드 공간 기준 근처 8개의 probe 의 sh 를 합산 후, 표면 N 방향으로 적분하여 diffuse 를 계산한다.

이후 프레임 ddgi cs : 첫 프레임과 동일하게 표면에서 직접광을 계산하고, 추가로 충돌지점 기준 근처 8개의 probe 의 sh 를 합산 후, 표면 N 방향으로 적분하여 diffuse 를 계산한다. (간접광 정보가 probe 에 누적되기 시작한다.)

이후 프레임 ps : 동일함 (probe 의 간접광 정보를 실제 렌더링 시에 사용하기 시작한다.)
...
첫 프레임 (probe 들에 저장된 라이팅 정보? radiance? 없음)

각 probe 에서 일정 개수만큼 ray tracing 실행

충돌 시 직접광 샘플링 (빛을 받았을때 그 표면의 색상 계산)

빛 방향에 장애물이 있다면 무시

probe 에 SH 형식으로 빛 정보를 저장

First frame: no indirect radiance stored in probes yet (cold start)

Each probe fires N rays distributed uniformly across the sphere
(Spherical Fibonacci sampling + random rotation per frame)

On ray hit : compute direct radiance at the hit surface
sample a light source, compute the reflected radiance at the hit surface
Also store hit distance per direction — used to detect and suppress contribution of occluded probes

Cast shadow ray toward the light source
if occluded, discard the contribution

Store accumulated ray results into per-probe L1 SH coefficients
...
From frame 2 onwards: indirect radiance is now available in probes

On ray hit: additionally sample indirect radiance from nearby probes
— blend probe SH by position, integrate over surface normal → indirect diffuse irradiance

Store accumulated ray results (direct + indirect) into per-probe L1 SH coefficients

Indirect light propagates further each frame → infinite bounces converge over time

Why DDGI uses SH:

- Compact: L1 SH encodes the full directional radiance distribution in only 12 floats (vs. 192 floats for an 8×8 texel grid)
- Fast lookup: evaluating irradiance for any surface normal N reduces to a single dot product — no texture sampling needed
- Linear blending: SH coefficients can be blended with a simple weighted sum, which is mathematically exact due to the linear natureof SH basis functions
- Sufficient for diffuse: diffuse radiance is a low-frequency directional signal — L1 SH captures it accurately enough
- Cache efficiency: SH is accessed ~960M times/sec at render time (pixels × 8 probes × 60fps) — 12 floats fits in cache far better than a texel grid

...
## 3.10 리소스 상태와 배리어 (Resource States & Barriers)

### 리소스 상태란

GPU 리소스(텍스처, 버퍼)는 현재 **어떤 용도로 사용 중인지**를 나타내는 상태를 가진다.
같은 텍스처라도 렌더 타겟으로 쓸 때와 셰이더에서 읽을 때는 GPU 내부 처리 방식이 다르다.

DX12는 이 상태를 개발자가 **직접 명시적으로 관리**해야 한다.
(DX11은 드라이버가 자동으로 추적했지만 성능 오버헤드가 컸다)

### 주요 리소스 상태

| 상태 | 의미 | 사용처 |
|------|------|--------|
| `RENDER_TARGET` | 그래픽 파이프라인이 픽셀을 쓰는 대상 | `RenderPassBegin`, Draw call 출력 |
| `SHADER_RESOURCE` | 셰이더에서 읽기 전용으로 접근 | SRV 바인딩, `texture.Sample()` |
| `UNORDERED_ACCESS` | 셰이더에서 읽기/쓰기 모두 가능 | UAV 바인딩, Compute Shader 출력 |
| `COPY_SRC` | 복사 명령의 소스 | `CopyResource`, `CopyTextureRegion` |
| `COPY_DST` | 복사 명령의 목적지 | `CopyResource`, upload 후 |
| `DEPTH_WRITE` | 깊이/스텐실 버퍼 쓰기 | DSV, 깊이 테스트 + 쓰기 |
| `DEPTH_READ` | 깊이 버퍼 읽기 전용 | depth-only shadow map 읽기 |
| `PRESENT` | Present 직전 상태 | `SwapChain::Present()` 전 |
| `COMMON` | 범용 초기 상태 | 리소스 생성 직후, 큐 간 공유 |

### 왜 상태 전환이 필요한가

GPU 하드웨어는 각 상태에 따라 내부 캐시와 메모리 레이아웃을 다르게 최적화한다.

```
RENDER_TARGET 상태:
  GPU 쓰기 캐시(ROP 캐시)에 데이터가 있을 수 있음
  메모리 레이아웃이 렌더링에 최적화됨

SHADER_RESOURCE 상태:
  셰이더 읽기 캐시(텍스처 캐시)에서 접근
  메모리 레이아웃이 샘플링에 최적화됨
```

상태를 전환하지 않고 `RENDER_TARGET`에 쓴 뒤 바로 `SHADER_RESOURCE`로 읽으면:
- 쓰기 캐시에 아직 flush되지 않은 데이터가 있을 수 있음
- 셰이더가 오래된 값을 읽게 됨 → **undefined behavior**

### 배리어 (Barrier)

배리어는 GPU에게 "이 리소스를 지금부터 다른 방식으로 사용할 것이므로 캐시를 flush하고 상태를 전환하라"고 알리는 명령이다.
...
// VizMotive 추상화 사용 시
device->Barrier(GPUBarrier::Image(&texture, ResourceState::RENDER_TARGET,
                                            ResourceState::SHADER_RESOURCE), cmd);
```

### 배리어 비용

배리어는 공짜가 아니다. 상태 전환 시 GPU가 다음 작업을 한다:

| 작업 | 설명 |
|------|------|
| 파이프라인 스톨 | 이전 작업이 완전히 끝날 때까지 대기 |
| 캐시 flush | 쓰기 캐시의 데이터를 메모리에 내림 |
| 캐시 무효화 | 읽기 캐시의 오래된 데이터를 무효화 |
| 레이아웃 변환 | 하드웨어에 따라 메모리 레이아웃 재구성 |

따라서 불필요한 배리어를 줄이는 것이 성능 최적화의 중요한 부분이다.

### 배리어 종류

```
Transition Barrier  — 상태 전환 (가장 일반적)
  RENDER_TARGET → SHADER_RESOURCE 등

UAV Barrier         — 같은 UAV 리소스에 대한 읽기/쓰기 충돌 방지
  Compute A가 UAV 쓰기 완료 → Compute B가 UAV 읽기 시작

Aliasing Barrier    — 같은 메모리 영역을 다른 리소스가 공유할 때
  고급 최적화, 일반적으로 잘 사용 안 됨
```

### Swapchain 버퍼의 상태 전환

```
프레임 시작:  PRESENT → RENDER_TARGET   (blit으로 쓰기 직전)
blit 완료:    RENDER_TARGET → PRESENT   (Present() 직전)
```

`DXGI_USAGE_SHADER_INPUT`이 있는 경우 추가 전환 가능:
```
RENDER_TARGET → SHADER_RESOURCE   (셰이더에서 읽기 — CopyResource 전 등)
SHADER_RESOURCE → COPY_SRC        (CopyResource 소스로 사용)
```

### 배리어 배칭 (Batching)

여러 배리어를 한 번에 묶어서 제출하면 GPU가 가능한 경우 병렬로 처리할 수 있다.
...
VizMotive에서는 `PushBarrier()` / `FlushBarriers()` 패턴으로 이를 구현한다:

```cpp
// 배리어를 모아두고
device->Barrier(GPUBarrier::Image(&texA, ...), cmd);
device->Barrier(GPUBarrier::Image(&texB, ...), cmd);
// 한 번에 flush
// (내부적으로 묶어서 ResourceBarrier 1회 호출)
```
...
| Fence | CPU-GPU 동기화 |
| Resource State | 리소스의 현재 사용 방식 (RENDER_TARGET, SHADER_RESOURCE 등) |
| Barrier | 리소스 상태 전환 명령 — 캐시 flush + 상태 변경 |
| Swap Chain | 화면 출력 버퍼 |

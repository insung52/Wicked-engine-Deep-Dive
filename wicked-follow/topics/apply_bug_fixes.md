...
VizMotive는 Windows DX12 전용이므로 Linux hang은 직접 해당하지 않는다.
그러나 WickedEngine과 동일하게 `WaitQueue()` API와 `wait_queues` 벡터가 존재할 경우,
같은 구조적 문제를 내포한다.

**GraphicsDevice_DX12.h 확인 포인트:**
```cpp
// 제거 대상: wait_queues 벡터
std::vector<...> wait_queues;

// 제거 대상: WaitQueue 선언
virtual void WaitQueue(CommandQueue queue) = 0;
```

### 해결

- `WaitQueue()` 함수 및 선언 제거
- `wait_queues` 벡터 제거
- Wetmap 처리를 별도 큐 대신 **동일 커맨드리스트**에서 수행하도록 변경

```cpp
// 변경 전: 별도 큐에서 wetmap 처리
WaitQueue(CommandQueue::COMPUTE);
// wetmap compute dispatch...

// 변경 후: 동일 커맨드리스트에서 처리 (큐 간 동기화 제거)
// wetmap compute dispatch (같은 cmd에서)
```

> **핵심**: DX12에서 큐 간 동기화는 fence로 충분하다.
> 별도 Wait API가 필요한 경우 대부분 설계 문제를 의미한다.

...
**`CreateBufferZeroed` 헬퍼**: `GBackendDevice.h:267`에 이미 정의되어 있음. ✅

**INDIRECT_ARGS 버퍼 생성 위치 실제 확인**:

```
SceneUpdate_Detail.cpp:1263  device->CreateBufferZeroed(&buf, &ddgi.rayBuffer)         ✅ 적용됨
SceneUpdate_Detail.cpp:1269  device->CreateBufferZeroed(&buf, &ddgi.varianceBuffer)     ✅ 적용됨
SceneUpdate_Detail.cpp:1276  device->CreateBufferZeroed(&buf, &ddgi.raycountBuffer)     ✅ 적용됨
SceneUpdate_Detail.cpp:1283  device->CreateBufferZeroed(&buf, &ddgi.rayallocationBuffer)✅ 적용됨
SceneUpdate_Detail.cpp:1290  device->CreateBufferZeroed(&buf, &ddgi.probeBuffer)        ✅ 적용됨

SortLib.cpp:46               device->CreateBuffer(&bd, nullptr, &indirectBuffer)        ⚠️ 미적용
ShaderEngine.cpp:137         CreateBuffer (INDIRECT_ARGS)                               ⚠️ 미적용
RenderPath3D_Detail.cpp:703  CreateBuffer (INDIRECT_ARGS)                               ⚠️ 미적용
RenderPath3D_Detail.cpp:2193 CreateBuffer (INDIRECT_ARGS)                               ⚠️ 미적용
GaussianSplatting_Detail.cpp:30  CreateBuffer (INDIRECT_ARGS)                           ⚠️ 미적용
```

**결론**: `CreateBufferZeroed`는 존재하고 DDGI 버퍼에는 적용됐지만, 나머지 INDIRECT_ARGS 버퍼 5곳이 아직 plain `CreateBuffer`를 사용 중. **부분 적용 상태**.
...
**GBackend.h `ResourceState` enum** (GBackend.h:459) 실제 내용:
...
**GraphicsDevice_DX12.cpp `_ParseResourceState()`** (line 76): `SWAPCHAIN` case 없음.
대신 Present 배리어는 `D3D12_RESOURCE_STATE_PRESENT`를 직접 하드코딩으로 처리 (line 6111, 6118).

**결론**: `SWAPCHAIN` 값이 enum에 없고, `_ParseResourceState()`에도 매핑 없음. **미적용**.
...
`StringConvert` 자체는 DX12와 무관한 범용 C++ 유틸리티다.
그러나 DX12는 **wide string (`wchar_t`)을 요구하는 API**가 존재한다:

```cpp
// DX12 디버그 오브젝트 이름 설정
resource->SetName(L"MyVertexBuffer");   // wchar_t 필요

// 파일 경로 (Windows API)
D3DCompileFromFile(L"shader.hlsl", ...);  // wchar_t 필요
```

엔진 내부는 `std::string` (char) 기반이므로, DX12 호출 시 변환이 필요하다.
이 변환에 버그가 있으면 디버그 이름 오염, 파일 경로 오류 등으로 이어진다.
...
`EngineCore/Utils/Helpers.h`에 `StringConvert` 구현 존재. 실제 코드:

```cpp
// _WIN32 경로: MultiByteToWideChar 사용 → 안전 ✅
inline void StringConvert(const std::string& from, std::wstring& to)
{
#ifdef _WIN32
    int num = MultiByteToWideChar(CP_UTF8, 0, from.c_str(), -1, NULL, 0);
    if (num > 0) { to.resize(num - 1); MultiByteToWideChar(...); }
#else
    std::wstring_convert<std::codecvt_utf8<wchar_t>> cv;  // deprecated ⚠️
    to = cv.from_bytes(from);
#endif
}

// 포인터 버전 (char* → wchar_t*): _WIN32는 안전, #else는 버그 있음
inline int StringConvert(const char* from, wchar_t* to, int dest_size_in_characters = -1)
{
#ifdef _WIN32
    // MultiByteToWideChar 사용 → 안전 ✅
#else
    auto result = cv.from_bytes(from).c_str();  // ⚠️ Dangling Pointer!
    // 임시 객체 소멸 후 result 접근 → UB
#endif
}
```

**결론**: `_WIN32` 빌드(현재 VizMotive)는 안전한 Windows API를 사용. **실질적 버그 없음**.
단, `#else` 분기(Linux/Mac)에는 `wstring_convert` 사용 + Dangling Pointer 버그가 그대로 남아있음. ⚠️ 크로스플랫폼 이식 시 주의.
...
`GraphicsDevice_DX12.cpp:5253` 실제 코드:
...
case SubresourceType::UAV:
    if (subresource < 0)
        ...
        return internal_state->subresources_uav[subresource].index;  // ⚠️ 범위 검사 없음
    break;
```

**결론**: `size()` 체크 없음. 잘못된 `subresource` 인덱스 전달 시 out-of-bounds 크래시 발생 가능. **미적용**.
...
`GraphicsDevice_DX12.cpp:7058` 실제 코드:

```cpp
void GraphicsDevice_DX12::CopyTexture(const Texture* dst, ..., const Texture* src, ...)
{
    auto src_internal = to_internal((const GPUBuffer*)src);  // ⚠️ UB 캐스팅 그대로!
    auto dst_internal = to_internal((const GPUBuffer*)dst);  // ⚠️ UB 캐스팅 그대로!
    // ...
}
```

**결론**: Strict Aliasing Rule 위반 캐스팅이 그대로 남아있음. **미적용**.

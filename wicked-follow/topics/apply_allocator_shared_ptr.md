...
#### "소멸 순서가 고정이면 WickedEngine 방식도 괜찮지 않나?"

VizMotive의 DLL 소멸 순서는 `VzEngineManager.cpp`의 `DeinitEngineLib()`에서 확인할 수 있다:

```cpp
// VzEngineManager.cpp:1200
bool DeinitEngineLib()
{
    vzcompmanager::DestroyAll();           // ① GPU 리소스 포함 모든 컴포넌트 소멸
    graphicsBackend.pluginDeinitializer(); // ② backend (DX12.dll) 소멸
    shaderEngine.pluginDeinitializer();    // ③ shaderEngine 소멸
}
```

이것은 **DLL 파일 언로드 순서가 아니라, 명시적 Deinitialize 함수 호출 순서**다.

```
① DestroyAll(): Texture, GPUBuffer 등 GPU 리소스의 internal_state → 전부 nullptr
② backend Deinit(): allocator 소멸 ← 이 시점엔 이미 shared_ptr가 없음
→ 소멸 시점은 안전
```

**그러나 소멸 시점이 안전해도 WickedEngine 방식은 쓸 수 없다.**
문제는 소멸 시점이 아니라 **런타임 도중** Engine.dll에서 Texture를 복사할 때 발생한다:
...
TextureComponent.cpp에서 graphics::Texture texture = resource.GetTexture();
  ...
  → 크래시  (DestroyAll과 무관하게, 정상 동작 중에 발생)
```
...
Allocator.h에는 allocator 등록/조회용 배열도 포함되어 있다:

```cpp
// Allocator.h:74 — inline이라 DLL마다 별도 인스턴스
inline SharedBlockAllocator* block_allocators[256] = {};
inline std::atomic<uint8_t> next_allocator_id{ 0 };
```

각 `SharedBlockAllocatorImpl`이 생성될 때 이 배열에 자신을 등록한다(`register_shared_block_allocator`).
그러나 **실제 shared_ptr의 inc/dec_refcount는 이 배열을 조회하지 않고 직접 포인터를 사용한다**:

```cpp
void reset() {
    allocator->dec_refcount(ptr);  // ← this->allocator 직접 역참조, 배열 조회 없음
}
```

`block_allocators[]`가 inline이라 DLL마다 별도 인스턴스이지만,
refcount 경로에서 사용되지 않으므로 DLL 경계 문제를 일으키지 않는다.

...
## VizMotive Allocator.h의 구현: RawStruct

> WickedEngine도 유사한 RawStruct 구조를 사용하지만, `alignas(256)` 사용 이유와
> shared_ptr 구현 방식이 다르다. 두 엔진의 차이점은 이 섹션에서 함께 설명한다.

### 슬롯 하나의 구조

```cpp
// VizMotive: Allocator.h
...
### alignas(256): WickedEngine과 VizMotive의 이유가 다르다

#### WickedEngine: 포인터 하위 8비트에 ID를 숨기기 위해
...
이렇게 8바이트 하나로 포인터와 ID를 함께 저장하기 때문에,
WickedEngine에서 `alignas(256)`은 **이 트릭의 전제 조건**이다.

#### VizMotive: 포인터 트릭은 쓰지 않는다

VizMotive는 shared_ptr에 `allocator*`를 별도 멤버로 저장한다 (16바이트):

```cpp
struct shared_ptr {
    T*                    ptr;        // 슬롯 포인터
    SharedBlockAllocator* allocator;  // allocator 포인터 (별도 멤버)
};
```

하위 8비트가 0일 필요가 없으므로, `alignas(256)`이 필수는 아니다.
그러나 **캐시 라인 정렬** 효과가 있어 그대로 유지한다.
...
이 엔진의 RawStruct는 다르다:

```
make_shared<Texture_DX12>() 내부:

  free_list에서 이미 확보된 슬롯 꺼냄 (힙 할당 없음):
  ┌────────────────────────────┐
  │ data[sizeof(Texture_DX12)] │  ← 데이터 (placement new로 생성자만 호출)
  ├────────────────────────────┤
...
// 이 엔진: ptr에서 RawStruct로 역산 (같은 슬롯 안에 있으므로)
...
```
make_shared<Texture_DX12>() 호출:
  1. locker.lock()
  2. free_list.back() → 슬롯 가져오기
  3. free_list.pop_back()
  4. locker.unlock()
  5. new (slot) Texture_DX12()  ← placement new
...
이 엔진에는 두 가지 allocator가 있다:

| | `SharedBlockAllocatorImpl<T, 256>` | `SharedHeapAllocator<T>` |
|---|---|---|
| 메모리 관리 | 풀링 (256슬롯 블록) | 개별 힙 할당 (`new`/`delete`) |
| 사용 대상 | 빈번하게 생성/삭제되는 객체 | 디바이스당 1개, 오래 유지되는 객체 |
| 예시 | `Texture_DX12`, `Resource_DX12` 등 | `AllocationHandler` |
| 생성 함수 | `make_shared<T>()` | `make_shared_single<T>()` |

`AllocationHandler`에 블록 풀링을 쓰면 256슬롯 중 1개만 사용하고 나머지가 낭비된다.
→ 이런 단일 객체에는 힙 직접 할당이 오히려 효율적.

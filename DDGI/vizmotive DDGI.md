...
- 결과 : 메시 외부 : 빨간색, 메시 내부 : 검은색 (비정상! 반대로 나옴)

- Green 채널: NdotL (초록색 밝기 = 조명 강도)

- 결과(외부) : 메시 근처 probe 들 중, 조명 반대편 probe 가 초록색, 조명쪽 probe 가 검은색 (비정상)
- 메시 내부 : 조명이랑 가까운쪽 probe 가 초록색 (외부랑 반대)

- Blue 채널: Normal Y (파란색 밝기 = hit 한 표면의 normal의 위/아래 방향)

- 결과(외부) : 바닥 메시 아래 probe 가 파란색, 위 probe 가 검은색 (비정상)
- 메시 내부 : 파란색 (비슷함, 차이 판별 어려움)

---

backface true -> -N 적용됨(surfaceHF.hlsli:390).

이 코드의 원래 역할 : 메시 내부의 probe가 메시의 뒷면에 hit 시, 해당 표면의 N 은 메시 밖으로 나가는 방향, 즉 메시 내부에서 볼 때의 표면 방향과 반대.

따라서 정확한 조명 계산을 위해 N 을 반전시킴.
